import*as N from"csv-stream";import{open as D}from"node:fs/promises";import*as I from"post-entity";import A from"better-sqlite3";import{ensureFileSync as C}from"fs-extra";var d={},p=n=>(d[n]===void 0&&(C(n),d[n]=new A(n),d[n].exec("PRAGMA journal_mode = OFF;"),d[n].exec("PRAGMA synchronous = 0;"),d[n].exec("PRAGMA cache_size = 1000000;"),d[n].exec("PRAGMA locking_mode = EXCLUSIVE"),d[n].exec("PRAGMA temp_store = MEMORY;")),d[n]),R=n=>{d[n]=void 0},T=(n,t,e=void 0)=>{let s=n.prepare(t);return e===void 0?s.run():s.bind(e).run()},h=(n,t,e={})=>n.prepare(t).bind(e).all();import O from"wink-nlp";import y from"wink-eng-lite-web-model";import{rmSync as b}from"fs-extra";var _=/'/g,U=/[\n\r]/g,W=1e3*60*60,L=O(y),c={createWords:[`CREATE TABLE IF NOT EXISTS words (
    ...
    FROM word_history
    WHERE account_id = :account_id
    ORDER BY hour ASC;`,getWords:"SELECT id, word FROM words;"},f=class{constructor(t=__dirname+"/../../../assets/wordCache.db",e=!0){this.cache={};this.path=t,e&&this.connect(t)}connect(t=this.path){return this.connection=p(t),c.createWords.forEach(e=>this.connection.exec(e)),c.createWordHistory.forEach(e=>this.connection.exec(e)),c.createAccounts.forEach(e=>this.connection.exec(e)),this.populate(),this}getUserId(t){if(typeof t=="number")return t;let e=h(this.connection,c.getUser,{username:t});return e.length>0?e[0].id:T(this.connection,c.insertUser,{username:t}).lastInsertRowid}getReport(t){let e=h(this.connection,c.getUniqueWords,{account_id:t});if(e.length===0)return e;let s=h(this.connection,c.getWordHistory,{account_id:t}),r=e[0].hour,a=e[e.length-1].hour+1,o=[],i=[],u=[];for(;r<a;r+=W){o.push(r);let E=e.find(m=>m.hour===r);E===void 0?(i.push(0),u.push(0)):(i.push(E.uniqueCount),u.push(E.count))}let g=[];return o.map((E,m)=>{let l=0;return s.filter(w=>w.hour===E).forEach(w=>{g.indexOf(w.word_id)===-1&&(l+=1,g.push(w.word_id))}),{hour:E,count:u[m],uniqueCount:i[m],ratio:u[m]===0?0:i[m]/u[m],newGrammar:l}})}reset(){return this.connection&&this.connection.close(),R(this.path),b(this.path),this}addUsages(t,e,s){let r=this.getUserId(e),a=s instanceof Date?s.toISOString().slice(0,-5):s;for(let o of Object.keys(t)){let i=t[o];for(let u=0;u<i.amount;u++)this.addUsage(i.id,r,a)}return r}addUsage(t,e,s){return T(this.connection,c.insertNewWordHistory,{word_id:t,account_id:e,timestamp:s}),this}fetch(t){let e=this.tokenize(t),s=[],r={};for(let o of e)this.cache[o]===void 0?s.push(o):(r[o]===void 0&&(r[o]={id:this.cache[o],amount:0}),r[o].amount+=1);let a=this.getsert(s);for(let o of a)this.cache[o.word]=o.id,r[o.word]===void 0&&(r[o.word]={id:this.cache[o.word],amount:0}),r[o.word].amount+=1;return r}tokenize(t){return L.readDoc(t.toLowerCase().replace(_,"").replace(U," ")).tokens().out()}populate(){let t=h(this.connection,c.getWords);for(let e of t)this.cache[e.word]=e.id;return this}getsert(t){let e=t instanceof Array?t:this.tokenize(t);if(e.length===0)return[];let s=`('${e.join("'), ('")}')`;T(this.connection,c.insertNewWords.replace("$$",s));let r=`('${e.join("', '")}')`;return h(this.connection,c.getNewWords.replace("$$",r),{})}};var S=process.argv[2]||"";if(S==="")throw new RangeError;var M={delimiter:"	",endLine:`
`,columnOffset:0,escapeChar:'"',enclosedChar:'"'},x=N.createStream(M);async function q(){let n=new f(`${process.cwd()}/tests/test.db`,!1).reset().connect(),t;(await D(S,"r")).createReadStream().pipe(x).on("data",function(r){if(r.retweet!=="False"||r.language!=="en")return;let a=r.tweet.toLowerCase();console.log(r)}).on("close",function(){let r=n.getReport(t);console.log(r)})}q();
